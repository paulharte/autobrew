...
def get_template(template_file):
    ...
    try:
        if template_file.startswith("http"):
            raise RuntimeError(
                "please install urllib and update cloudFormationValidator!"
            )
                    ...
                    if isinstance(atr_val, dict) and atr_val.keys()[0] not in "Ref":
                        ...
                        for o in atr_val:
                            if not isinstance(o, dict):
                                ...
                                print(o)
                                return False
                    elif (not isinstance(atr_val, dict)) and (
                        not isinstance(atr_val, list)
                    ):
                        print("Not Refference Attribute: ", atr_key)
                        return False
...
def main(arguments):
    ...
    (
        ...
        allow_additional_attributes,
        not_allow_attributes,
    ) = get_configuration(args.cf_rules)
