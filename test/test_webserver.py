...
class TestWebserver(TestCase):
    ...
    def _test_get_measurements(self, client):
        ...
        response = client.get(
            "/live_temperature?name=" + PROBE_PREFIX + STUB_BREW_TEMP_NAME
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_nickname(self):
        ...
        response = client.get("/brews/new?name=mynewbrew")
        self.assertEqual(response.status_code, 200)
        series = meas_service.new_series(
            brew_service.get_active(), source_name, SeriesType.TEMPERATURE
        )
        ...
        self.assertEqual(response.status_code, 200)
        self.assertIn(
            "successfully updated nickname to my_nickname", str(response.data)
        )
    ...
    def test_primary(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertIn(
            "successfully set as primary temperature source", str(response.data)
        )
